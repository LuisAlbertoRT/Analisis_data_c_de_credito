...
# 1. Aprender los bordes de los deciles SOLO en X_train
_, bin_edges = pd.qcut(
    X_train['amount_log'], 
    q=10, 
    labels=False, 
...
# 2. Aplicar los MISMOS bordes a X_train y a X_test usando pd.cut
...
# ==============================================================================
# 1. DIVIDIR TRAIN EN TRAIN INTERNO Y VALIDACIÓN
# ==============================================================================
...
# A. Evaluar los 3 modelos individuales
for nombre, model in modelos.items():
...
# B. Evaluar el Ensamble (90% XGB + 10% RF)
...
# ==============================================================================
# 4. IMPRIMIR RESULTADOS Y GRÁFICO FINAL
# ==============================================================================
...
# 1. Obtener probabilidades predichas del modelo XGBoost en el set de VALIDACIÓN
y_probs_val = best_xgb.predict_proba(X_val)[:, 1]

# 2. Calcular Precision, Recall y Umbrales usando SOLO datos de VALIDACIÓN
precisions, recalls, thresholds = precision_recall_curve(y_val, y_probs_val)

# 3. Calcular F1-Score para encontrar el umbral óptimo sin data leakage
...
# 4. Obtener probabilidades en el set de TEST y aplicar el umbral prefijado
y_probs_xgb = best_xgb.predict_proba(X_test)[:, 1]
y_pred_optimo = (y_probs_xgb >= best_threshold).astype(int)

# 5. Reporte de Clasificación en TEST con el umbral imparcial
...
# 6. Graficar la Matriz de Confusión en TEST
...
# ==============================================================================
# 2. CÁLCULO DEL UMBRAL ÓPTIMO VÍA OUT-OF-FOLD (SOLO TRAIN - SIN DATA LEAKAGE)
# ==============================================================================
...
# ==============================================================================
# 1. OBTENER PROBABILIDADES DE AMBOS MODELOS EN VALIDACIÓN
# ==============================================================================
# Se calculan sobre X_val / y_val para no contaminar Test
probs_xgb_val = final_xgb_model.predict_proba(X_val)[:, 1]
probs_rf_val = best_rf.predict_proba(X_val)[:, 1]

# ==============================================================================
# 2. ENCONTRAR LA MEJOR PONDERACIÓN (PESOS DEL ENSEMBLE) EN VALIDACIÓN
# ==============================================================================
best_w_xgb = 0.5
best_pr_auc_val = 0
best_val_blend_probs = None

# Probar distintas combinaciones de pesos sobre el conjunto de VALIDACIÓN
for w_xgb in np.linspace(0.1, 0.9, 9):
    w_rf = 1.0 - w_xgb
    blend_probs = (w_xgb * probs_xgb_val) + (w_rf * probs_rf_val)
    score = average_precision_score(y_val, blend_probs)

    if score > best_pr_auc_val:
        best_pr_auc_val = score
        best_w_xgb = w_xgb
        best_val_blend_probs = blend_probs

print(
    f"=== PESOS ÓPTIMOS (CALCULADOS EN VALIDACIÓN): XGBoost ({best_w_xgb:.2f}) + Random Forest"
    f" ({1-best_w_xgb:.2f}) ==="
)
print(f"PR-AUC del Ensamble en Validación: {best_pr_auc_val:.4f}\n")

# ==============================================================================
# 3. CÁLCULO DEL UMBRAL ÓPTIMO DEL ENSAMBLE EN VALIDACIÓN
# ==============================================================================
precisions, recalls, thresholds = precision_recall_curve(
    y_val, best_val_blend_probs
)
f1_scores = 2 * (precisions * recalls) / (precisions + recalls + 1e-10)
best_idx = np.argmax(f1_scores)
OPTIMAL_THRESHOLD_ENSEMBLE = float(thresholds[best_idx])

print(f"=== UMBRAL ÓPTIMO ENSEMBLE (VALIDACIÓN): {OPTIMAL_THRESHOLD_ENSEMBLE:.4f} ===")
print(f"Precisión esperada: {precisions[best_idx]:.4f}")
print(f"Recall esperado:    {recalls[best_idx]:.4f}")
print(f"F1-Score máximo:     {f1_scores[best_idx]:.4f}\n")

# ==============================================================================
# 4. WRAPPER DE PRODUCCIÓN PARA EL ENSAMBLE
# ==============================================================================
...
# Instanciar el detector ensamble
...
# ==============================================================================
# 5. EVALUACIÓN Y MATRIZ DE CONFUSIÓN DEL ENSAMBLE EN TEST (SIN DATA LEAKAGE)
# ==============================================================================
y_pred_ensemble = ensemble_detector.predict(X_test)
probs_ensemble_test = ensemble_detector.predict_proba(X_test)
pr_auc_ensemble_test = average_precision_score(y_test, probs_ensemble_test)

print(f"=== PR-AUC EN TEST DEL ENSAMBLE: {pr_auc_ensemble_test:.4f} ===")
print("=== REPORTE DE CLASIFICACIÓN DEL ENSAMBLE EN TEST ===")
print(
    classification_report(
...
print("=== MATRIZ DE CONFUSIÓN DEL ENSAMBLE EN TEST ===")
...
# ==============================================================================
# 6. GUARDAR ARTEFACTO A DISCO
# ==============================================================================
os.makedirs("Salidas", exist_ok=True)
joblib.dump(ensemble_detector, 'Salidas/modelo_fraude_ensemble_xgb_rf.joblib')
print("\n✅ Ensamble guardado exitosamente en 'Salidas/modelo_fraude_ensemble_xgb_rf.joblib'")
...
# ==============================================================================
# 1. DIVISIÓN INTERNA DE VALIDACIÓN PARA EARLY STOPPING
# ==============================================================================
...
# ==============================================================================
# 2. ESPECIFICACIÓN Y ENTRENAMIENTO CON EARLY STOPPING (OPCIÓN 1)
# ==============================================================================
...
# ==============================================================================
# 3. EVALUACIÓN Y VALIDACIÓN DE OVERFITTING (TRAIN VS. TEST)
# ==============================================================================
...
# ==============================================================================
# 4. CÁLCULO DEL UMBRAL ÓPTIMO EN VALIDACIÓN (SIN DATA LEAKAGE)
# ==============================================================================
...
# ==============================================================================
# 5. CLASE ENCAPSULADORA PARA DESPLIEGUE EN PRODUCCIÓN (WRAPPER)
# ==============================================================================
class ProductionFraudClassifier:
    """Wrapper de producción que encapsula el modelo XGBoost y aplica
    automáticamente el umbral de decisión óptimo en las predicciones.
...
# ==============================================================================
# 6. REPORTE FINAL DE CLASIFICACIÓN EN TEST (EVALUACIÓN IMPARCIAL)
# ==============================================================================
y_pred_final = fraud_detector_final.predict(X_test)
...
# ==============================================================================
# 7. GUARDAR ARTEFACTO COMPLETO A DISCO
# ==============================================================================
os.makedirs("Salidas", exist_ok=True)
file_name = 'Salidas/modelo_fraude_xgboost_final.joblib'
joblib.dump(fraud_detector_final, file_name)
...
# ==============================================================================
# 1. EXTRACCIÓN Y CÁLCULO DE MÉTRICAS CONSOLIDADAS
# ==============================================================================
...
# ==============================================================================
# 2. TABLAS DE ESTADÍSTICA EXECUTIVA PARA EL REPORTE
# ==============================================================================
print("=" * 75)
...
# ==============================================================================
# 3. GENERACIÓN DE VISUALIZACIONES PARA DOCUMENTACIÓN (DASHBOARD)
# ==============================================================================
